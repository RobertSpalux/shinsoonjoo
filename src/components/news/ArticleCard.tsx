...
export default function ArticleCard({
  ...
  return (
    ...
    >
      <Link
        href={`/news/${slug}`}
        className="flex h-full flex-col rounded-2xl border border-[var(--color-line)] bg-[var(--color-ink-card)] p-7 transition-all duration-500 hover:-translate-y-1 hover:border-[var(--color-gold-dim)]/60 hover:shadow-xl hover:shadow-slate-300/60 md:p-8"
      >
        <div className="mb-4 flex items-center justify-between">
          <span
            className={`rounded-full border px-3 py-1 text-[11px] font-semibold tracking-wide ${
              CATEGORY_COLORS[category] ?? "text-slate-600 border-[var(--color-line)]"
            }`}
          >
            {category}
          </span>
        ...
        <h3 className="mb-3 text-lg font-bold leading-snug text-slate-900 transition-colors duration-300 group-hover:text-[var(--color-gold-light)]">
          {title}
        </h3>
