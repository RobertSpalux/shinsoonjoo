...
[브랜드 정의 — '보험 리모델링'. 근거 자료가 다르게 말해도 이 정의를 우선한다]
- 보험 리모델링은 '갈아타기'나 '승환'과 동의어가 아니다. 절대 등치시키지 않는다.
- 리모델링 = 가입한 모든 계약을 담보 단위로 펼쳐 합산하여 중복·과설계·보장 공백을 찾아내는 '점검'이다.
- 점검 결과의 대부분은 '유지'다. 해지·신규가입은 그중 일부 담보에 한정된다.
- 따라서 리모델링은 승환 신중론과 충돌하지 않는다. 오히려 무분별한 승환을 막는 장치다.
- '무조건 해지' 프레임 금지. '무조건 갈아타기' 프레임도 금지. 리모델링의 결론은 대부분 "이건 유지하세요"다.
