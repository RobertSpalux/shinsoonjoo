...
- title / naver_title / blogspot_title: 채널별 제목 3종 — 세 채널이 제목을 공유하지 않는다.
  ...
  · blogspot_title (구글): ⚠️ 본진과 블로그스팟은 둘 다 구글 색인 대상 — title과 같은 제목이면 자기잠식(cannibalization)이 일어난다. 반드시 title과 다른 각도·다른 어절 구성으로 쓰고, 같은 키워드를 그대로 반복하지 않는다. 질문형 또는 비교형으로 각도를 튼다.
  · naver_title (네이버): 호기심·공감 훅 중심. 구어체 허용. 숫자·질문을 활용해 클릭을 유도하되 과장·낚시는 금지.
...
export const FACTORY_OUTPUT_SCHEMA = {
  ...
  properties: {
    ...
    naver_title: {
      type: "string",
      description: "네이버 전용 제목 — 호기심·공감 훅 중심, 구어체 허용, 숫자·질문 활용. 과장·낚시 금지",
    },
    blogspot_title: {
      type: "string",
      description: "블로그스팟 전용 제목 — title과 반드시 다른 각도·다른 어절(구글 자기잠식 방지). 질문형/비교형",
    },
