...
- main_website_markdown: 본진 웹사이트용. 구글·AI검색이 최종 도달하는 '권위 문서'다 — 주제를 가장 포괄적으로 다룬다. 진지하고 전문적인 톤의 테크니컬 칼럼. 공백 포함 2,000~3,200자. 마지막에 "${BRAND.personName} ${BRAND.title}의 한 줄 조언" 블록인용(>)으로 마무리 — 일반론·교과서적 요약이 아니라 ${years}년 현장 경험의 1인칭 서술로, 상담실에서 실제로 본 것을 말하는 톤.
  · ⚠️ 조언 블록은 반드시 아래 형식 그대로 쓴다(제목 굵게 + 빈 인용 줄 '>'로 본문과 분리):
...
- title / naver_title / blogspot_title: 채널별 제목 3종 — 세 채널이 제목을 공유하지 않는다.
  · 제목 훅 유형(3종 공통) — 이 콘텐츠에 가장 맞는 하나를 골라 적용한다:
    ① 통념 반전형: 독자의 통념을 앞에 놓고 뒤집는다 (구조 예: "◯◯라고요? ◯◯부터입니다")
    ② 질문형: 독자가 스스로 답하지 못하는 질문 (예: 담보 합산 금액을 아는가)
    ③ 숫자 구체형: 구체 숫자로 약속한다 (개수·금액·기준 — 예: "반드시 확인할 4가지")
    ⚠️ 훅은 낚시가 아니다 — 본문이 지키지 못하는 약속·과장 금지. 아래 채널별 규칙(각도 분리·길이)은 그대로 유지하며 충돌 시 채널별 규칙이 우선한다.
...
⑦ 말미 "${BRAND.personName} ${BRAND.title}의 한 줄 조언" 블록인용(>)은 상록수에도 유지 (톤·형식 모두 공통 지침을 그대로 따른다 — 제목 굵게 + 빈 인용 줄로 본문 분리)
