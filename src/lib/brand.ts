...
export const CONDITIONAL_NOTICES = {
  /**
   * 개인 의견 귀속.
   * ⚠️ 바이럴(블로그 등)에서는 필수안내사항 블록이 아니라 **본문에 1회 이상** 노출해야 한다.
   */
  personalOpinion:
    "상기 내용은 모집종사자 개인의 의견이며, 계약체결에 따른 이익 또는 손실은 보험계약자 등에게 귀속됩니다.",
...
/**
 * 사이트 전체 1건 심의(§6.11-1)의 필수 유의문구 2종.
 * 준법팀 회신(2026-07-21) 지정 문구를 토씨 그대로 사용한다(축약·변경 불가).
 * 공통 푸터에 1회 배치해 전 페이지를 커버한다(§6.11-4).
 * ⚠️ #1은 회신 문구("보험설계사의 의견")로, 본문 삽입용 CONDITIONAL_NOTICES.personalOpinion
 *    ("모집종사자 개인의 의견")과 주어가 다르다 — 의도된 차이이므로 통일하지 말 것.
 * ⚠️ #2는 CONDITIONAL_NOTICES.policyReference와 자구가 동일하나, 푸터 필수문구는 독립 자산으로
 *    고정한다(본문용 상수가 나중에 바뀌어도 심의 통과본 자구에 새지 않도록).
 */
export const SITE_FOOTER_NOTICES = [
