 ...
 *
 * ⚠️ 미승인 상태 = null. 승인·심의필 수령 후 **이 상수 한 곳만** 채우면 푸터(전 페이지)에 실번호로
 *    자동 치환된다. (예: `{ authority: "프라임에셋", no: "2026-07-XXXX", from: "2026.07.__", to: "2027.07.__" }`)
 * - 필수안내사항은 **전 페이지 푸터에 상시노출**한다(심의 담당자 반송 의견, 2026-07-23). null인 동안
 *   푸터는 `renderMandatoryNotice(null, "submission")`로 **공란 예시(제_____호)** 를 상시 렌더한다.
 * - ⚠️ 임의 번호 생성 금지 — 공란은 밑줄 형식 그대로 유지한다(회사 참고자료: 예시는 공란 표기,
 ...
 * ⚠️ 필수 유의문구 2종(개인의견·약관참조)은 여기 두지 않는다 → REQUIRED_NOTICES(아래) 전용.
 *    선택(조건부)으로 오분류하면 누락된다(실제 사고: 약관 참조 문구 누락으로 3호 팜스 반송 2026-07-27).
 */
...
/**
 * 게시글마다 본문에 상시 노출하는 필수 유의문구 2종 (준법팀 회신 2026-07-21, §6.11-4·§6.11-7).
 * ⚠️ **필수다.** 소재 판단(CONDITIONAL_NOTICES)과 분리해 둔다 — 선택 삽입으로 취급하면 누락된다
 *    (실제 사고: 약관 참조 문구 누락으로 3호 팜스 반송 2026-07-27).
 * 정본 자구(회신 지정, §6.11-7). 축약·변형 금지(§6.10). **이 상수가 단일 소스**이며
 * 푸터·게시글 본문(ArticleNotice)·osmu(바이럴 본문) 3경로가 모두 이 배열을 참조한다.
 * 게시글에서 본문+푸터 이중 노출은 허용(유의문구 과다 반려 없음).
 */
